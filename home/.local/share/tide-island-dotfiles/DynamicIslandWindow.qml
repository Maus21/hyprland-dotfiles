...
PanelWindow {
    ...
    property string fallbackTideThemeId: "default"
    readonly property string tideThemeId: root.shellRootController && root.shellRootController.tideThemeId !== undefined
        ? root.shellRootController.tideThemeId
        : root.fallbackTideThemeId
    readonly property color tidePanelColor: tideThemeColor("panel")
    ...
    readonly property color tideTextSecondaryColor: tideThemeColor("muted")
    readonly property color tideSelectedTextColor: tideThemeColor("selectedText")
    readonly property color tideErrorColor: tideThemeColor("error")
    readonly property color tideWarningColor: tideThemeColor("warning")
    readonly property color tideSuccessColor: tideThemeColor("success")
    readonly property color tideInfoColor: tideThemeColor("info")

    function normalizeTideTheme(themeId) {
    ...
    function setTideTheme(themeId) {
        const normalized = normalizeTideTheme(themeId);
        root.fallbackTideThemeId = normalized;
        if (root.shellRootController && root.shellRootController.setTideThemeAll)
            root.shellRootController.setTideThemeAll(normalized);
    }

    function tideThemeColor(role) {
        const fallback = {
        ...
        const palette = root.shellRootController && root.shellRootController.tidePalette
            ? root.shellRootController.tidePalette
            : fallback;
        return palette[role] || fallback[role] || fallback.text;
    }
    ...
    readonly property real overviewWindowHeight: root.overviewVisible
        ...
        : 0
    readonly property bool appLauncherLayerVisible: root.toolOverlayMode === "launcher"
    readonly property bool themeSwitcherLayerVisible: root.toolOverlayMode === "theme"
    readonly property bool calculatorLayerVisible: root.toolOverlayMode === "calculator"
    readonly property bool webSearchLayerVisible: root.toolOverlayMode === "search"
    readonly property bool toolOverlayVisible: root.toolOverlayMode !== ""
    readonly property real toolOverlayWidth: root.appLauncherLayerVisible ? Math.min(root.width - 48, 760)
        : (root.themeSwitcherLayerVisible ? Math.min(root.width - 48, 620)
        : (root.calculatorLayerVisible ? Math.min(root.width - 48, 620)
        : (root.webSearchLayerVisible ? Math.min(root.width - 48, 680) : userConfig.islandWidth)))
    readonly property real toolOverlayHeight: root.appLauncherLayerVisible ? 520
        : (root.themeSwitcherLayerVisible ? 286
        : (root.calculatorLayerVisible ? 220
        : (root.webSearchLayerVisible ? 170 : userConfig.islandHeight)))
    readonly property real toolOverlayRadius: 24
    property string toolOverlayMode: ""
    ...
    WlrLayershell.layer: islandContainer.wallpaperPickerLayerVisible || root.toolOverlayVisible
        ? WlrLayer.Overlay
        : WlrLayer.Top
    WlrLayershell.keyboardFocus: {
        if (root.toolOverlayVisible)
    ...
    property string wallpaperPickerActiveWallpaper: root.shellRootController
    ...
    function showToolOverlay(mode) {
        root.toolOverlayMode = mode;
        islandContainer.showToolOverlay();
    }

    function toggleAppLauncherWindow() {
        if (root.appLauncherLayerVisible)
            root.closeToolOverlay();
        else
            root.showToolOverlay("launcher");
    }

    function toggleThemeSwitcherWindow() {
        if (root.themeSwitcherLayerVisible)
            root.closeToolOverlay();
        else
            root.showToolOverlay("theme");
    }

    function toggleCalculatorWindow() {
        if (root.calculatorLayerVisible)
            root.closeToolOverlay();
        else
            root.showToolOverlay("calculator");
    }

    function toggleWebSearchWindow() {
        if (root.webSearchLayerVisible)
            root.closeToolOverlay();
        else
            root.showToolOverlay("search");
    }
    ...
    FocusScope {
        ...
        readonly property bool toolOverlayLayerVisible: !root.overviewVisible
            && islandState === "tool_overlay"
            && root.toolOverlayVisible
        ...
        CompositorWorkspaceTracker {
            ...
            onWorkspaceActivated: function(workspaceId) {
                // Workspace activation is state synchronization only.  The
                // dedicated overview still reads currentWs, but changing
                // workspace must not replace the resting island capsule.
                islandContainer.currentWs = workspaceId;
            }
        }
        ...
        Timer {
            ...
            onTriggered: {
                ...
                if (nextRemainingSeconds <= 0) {
                    islandContainer.finishTimerCountdown();
                } else {
        ...
        Rectangle {
            ...
            color: root.overviewContentVisible ? root.overviewCapsuleColor : root.tidePanelColor
            ...
            Rectangle {
                ...
                color: StyleTokens.transparent
                border.width: 1
                border.color: root.tideBorderColor
            ...
            Loader {
                ...
                sourceComponent: Component {
                    OsdLayer {
                        ...
                        borderColor: root.tideBorderColor
                        accentColor: root.tideAccentColor
                        textPrimary: root.tideTextPrimaryColor
                        transitionProgress: islandContainer.swipeTransitionProgress
                        slideDirection: islandContainer.splitOriginSide
            ...
            Loader {
                ...
                sourceComponent: Component {
                    ExpandedPlayerLayer {
                        ...
                        borderColor: root.tideBorderColor
                        accentColor: root.tideAccentColor
                        accentAltColor: root.tideAccentAltColor
                        textPrimary: root.tideTextPrimaryColor
                        textSecondary: root.tideTextSecondaryColor
            ...
            Loader {
                ...
                sourceComponent: Component {
                    BluetoothExpandedLayer {
                        ...
                        textPrimary: root.tideTextPrimaryColor
                        textSecondary: root.tideTextSecondaryColor
                        errorColor: root.tideErrorColor
                        warningColor: root.tideWarningColor
                        successColor: root.tideSuccessColor
            ...
            Loader {
                ...
                sourceComponent: Component {
                    ControlCenterLayer {
                        ...
                        cardAccentAlt: root.tideAccentAltColor
                        selectedTextColor: root.tideSelectedTextColor
                        errorColor: root.tideErrorColor
                        warningColor: root.tideWarningColor
                        successColor: root.tideSuccessColor
            ...
            Loader {
                ...
                anchors.fill: parent
                active: islandContainer.toolOverlayLayerVisible && root.appLauncherLayerVisible
                asynchronous: false
                visible: active
                ...
                sourceComponent: Component {
                    AppLauncherLayer {
                        ...
                        textSecondary: root.tideTextSecondaryColor
                        selectedTextColor: root.tideSelectedTextColor
                        showCondition: islandContainer.toolOverlayLayerVisible && root.appLauncherLayerVisible
            ...
            Loader {
                ...
                anchors.fill: parent
                active: islandContainer.toolOverlayLayerVisible && root.themeSwitcherLayerVisible
                asynchronous: false
                visible: active
                ...
                sourceComponent: Component {
                    ThemeSwitcherLayer {
                        textFontFamily: root.textFontFamily
                        currentThemeId: root.tideThemeId
                        panelColor: root.tidePanelColor
                        moduleColor: root.tideModuleColor
                        moduleHover: root.tideModuleHoverColor
                        borderColor: root.tideBorderColor
                        ...
                        textSecondary: root.tideTextSecondaryColor
                        selectedTextColor: root.tideSelectedTextColor
                        showCondition: islandContainer.toolOverlayLayerVisible && root.themeSwitcherLayerVisible
                        onCloseRequested: root.closeToolOverlay()
                        onThemeApplied: function(themeId) {
                            root.setTideTheme(themeId);
                            if (root.shellRootController && root.shellRootController.syncCurrentWallpaper)
                                root.shellRootController.syncCurrentWallpaper(true);
                        }
                    }
                }
            }
            ...
            Loader {
                id: calculatorLoader
                anchors.fill: parent
                active: islandContainer.toolOverlayLayerVisible && root.calculatorLayerVisible
                asynchronous: false
                visible: active
                ...
                sourceComponent: Component {
                    CalculatorLayer {
                        ...
                        textPrimary: root.tideTextPrimaryColor
                        textSecondary: root.tideTextSecondaryColor
                        showCondition: islandContainer.toolOverlayLayerVisible && root.calculatorLayerVisible
                        onCloseRequested: root.closeToolOverlay()
                    }
                }
            }
            ...
            Loader {
                id: webSearchLoader
                anchors.fill: parent
                active: islandContainer.toolOverlayLayerVisible && root.webSearchLayerVisible
                asynchronous: false
                visible: active
            ...
            Loader {
                ...
                sourceComponent: Component {
                    WorkspaceOverviewScene {
                        ...
                        activeBorderColor: root.tideAccentColor
                        panelColor: root.tidePanelColor
                        moduleColor: root.tideModuleColor
                        moduleHover: root.tideModuleHoverColor
                        borderColor: root.tideBorderColor
        ...
        Process {
            ...
            command: ["bash", "-lc", "cat \"$HOME/.cache/hypr-theme-switcher/current\" 2>/dev/null || printf default"]
            running: false
            stdout: SplitParser {
                onRead: data => root.setTideTheme(data)
            }
        }
        ...
        Item {
            ...
            Component.onCompleted: {
                mounted = islandContainer.timerBubbleWanted;
                reveal = mounted ? 1 : 0;
            }
            ...
            Canvas {
                ...
                anchors.fill: parent
                anchors.margins: 1
                property color trackColor: root.tideBorderColor
                property color progressColor: root.tideAccentColor
                property color completionColor: root.tideWarningColor
                ...
                onCompletionColorChanged: requestPaint()
                ...
                onPaint: {
                    ...
                    if (completionActive) {
                        if (flash > 0) {
                            ...
                            ctx.strokeStyle = colorWithAlpha(completionColor, 0.18 * flash);
                            ctx.arc(centerX, centerY, radius, 0, Math.PI * 2);
                            ctx.stroke();
                        }
                        ...
                        ctx.beginPath();
                        ctx.lineWidth = lineWidth;
                        ctx.strokeStyle = colorWithAlpha(completionColor, 0.72 + 0.28 * flash);
                        ctx.arc(centerX, centerY, radius, 0, Math.PI * 2);
                        ctx.stroke();
                    } else if (progress > 0) {
                        ctx.beginPath();
            ...
            Text {
                ...
                text: "󰔛"
                color: root.tideTextPrimaryColor
